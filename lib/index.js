...
/**
 * Takes care of deprecating the tag property.
 * @function
 * @param {var} - A property name variable.
 */
function deprecateTag(propertyName) {
  if (propertyName === 'tag') {
    console.warn('tag will be deprecated in v1.0.0');
 ...
 * @function
 * @param {object} Flexible configuration.
 */
function attachTags(conf) {
  ...
  if (Array.isArray(tag)) {
    for (var t = 0; t < tag.length; t = t + 1) {
      swaggerObject[propertyName].push(tag[t]);
    }
  } else {
